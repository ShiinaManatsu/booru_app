...
static std::string WideToUtf8(const wchar_t* wide) {
  ...
  (void) WideCharToMultiByte(
    CP_UTF8, 0, wide, -1, result.data(), len, nullptr, nullptr
  );
  return result;
}
  ...
  HRESULT __stdcall Activate(
    ...
    try {
      ...
      for (ULONG i = 0; i < count; i++) {
        auto item = data[i];
        const std::string key = WideToUtf8(item.Key);
        const std::string value = WideToUtf8(item.Value);
      ...
      const auto payload = string(WideToUtf8(args));
