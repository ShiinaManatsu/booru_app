...
void globalInitial() {
  accountOperation.where((_) => AppSettings.localUsers.contains((LocalUser user) => user.clientType == AppSettings.currentClient)).listen((event) => event());

  if (!kIsWeb && (Platform.isAndroid || Platform.isIOS)) {
    getUriLinksStream().listen((link) {
      if (AppSettings.currentClient == ClientType.Konachan && link.host.contains("yande")) {
        AppSettings.currentClient = ClientType.Yande;
        booruBloc.onReset.add(null);
      } else if (AppSettings.currentClient == ClientType.Yande && link.host.contains("konachan")) {
        AppSettings.currentClient = ClientType.Konachan;
        booruBloc.onReset.add(null);
      }
      ExtendedNavigator.root.push(
        routes.Routes.postViewPageByPostID,
        arguments: routes.PostViewPageByPostIDArguments(postID: link.pathSegments[link.pathSegments.indexOf("show") + 1]),
      );
    });
  }
  ...
  AppSettings.savePath.then((value) async {
    if (!kIsWeb && (Platform.isAndroid || Platform.isIOS)) {
      if (value == null || value.isEmpty) {
        AppSettings.setSavePath((await getExternalStorageDirectory()).absolute.path);
      }
    } else {
      if (!kIsWeb) AppSettings.setSavePath(p.join(Directory.current.absolute.path, "/BooruPhotos"));
    }
  });

  SharedPreferencesExtension.getTyped<String>("PreviewQuality").then((q) {
    if (q != null) {
      AppSettings.previewQuality = EnumToString.fromString(PreviewQuality.values, q);
    } else {
      AppSettings.previewQuality = PreviewQuality.Low;
      SharedPreferencesExtension.setTyped("PreviewQuality", EnumToString.convertToString(PreviewQuality.Low));
    }
  });
  ...
  SharedPreferencesExtension.getTyped<double>("masonryGridBorderRadius").then((value) {
    if (value != null) {
      AppSettings.masonryGridBorderRadius = value;
    } else {
      AppSettings.masonryGridBorderRadius = 12;
      SharedPreferencesExtension.setTyped<double>("masonryGridBorderRadius", 12);
    }
  });

  SharedPreferencesExtension.getTyped<double>("masonryGridSpacing").then((value) {
    if (value != null) {
      ...
      child: MaterialApp(
        builder: (context, child) {
          ...
          return MediaQuery(
            ...
            child: ExtendedNavigator<routes.Router>(router: routes.Router()),
          );
        },
        title: 'Home',
        darkTheme: darkTheme,
